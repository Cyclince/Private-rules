...
import type { DomainRule, DomainRuleType, GeoSourceSuggestion, ImportPreview, RuleCategory, RuleConflict, RuleOptimizationMode } from '../../types/domain-rules';
...
export function RulesPanel({ api, categories, category, onSelectCategory, onToast }: Props) {
  ...
  const [replacingRule, setReplacingRule] = useState(false);
  ...
  if (!category) {
    ...
    return <div className="page-stack unified-page">
      <header className="page-title"><div><span className="eyebrow">RULE LIBRARY</span><h1>规则汇总</h1><p>从零维护规则，或聚合多个上游来源继续处理</p></div>{api.meta.authType !== 'telegram' && <button className="primary-action title-action" onClick={() => setShowCreate(true)}><UiIcon name="plus" size={19}/>新建规则</button>}</header>
  ...
  async function add(replaceConflicts = false) {
    setReplacingRule(replaceConflicts);
    try {
      const result = await api.addRule(currentCategory.id, { value, type: type || undefined, note, replaceConflicts });
      if (result.conflicts.length) { setRuleConflicts(result.conflicts); return; }
      setRuleConflicts([]); setValue(''); setNote('');
      onToast(result.replaced ? `已替换 ${result.replaced} 条重复或冲突规则` : '规则已添加');
    } finally { setReplacingRule(false); }
  }
  async function previewImport() { const result = await api.importPreview(currentCategory.id, bulkText); setPreview(result.preview); }
  async function confirmImport() { const replacing = Boolean(preview?.conflicts?.length); await api.confirmImport(currentCategory.id, bulkText, replacing); setBulkText(''); setPreview(null); onToast(replacing ? '冲突规则已替换，批量导入完成' : '批量导入完成'); }
  ...
  return <div className="page-stack unified-page">
    ...
    {preview && createPortal(<div className="rules-dialog-backdrop" onMouseDown={() => setPreview(null)}><section className="bulk-preview-dialog" role="dialog" aria-modal="true" aria-labelledby="bulk-preview-title" onMouseDown={(event) => event.stopPropagation()}><div className="builder-head"><div><h2 id="bulk-preview-title">批量导入预览</h2><p>确认规则类型和重复项，导入后仍可逐条调整</p></div><button className="dialog-close-button" aria-label="关闭导入预览" onClick={() => setPreview(null)}><UiIcon name="close" size={18}/></button></div><div className="import-preview-summary"><span><strong>{preview.rules.length}</strong><small>可导入</small></span><span><strong>{preview.duplicateValues.length}</strong><small>输入内重复</small></span><span><strong>{preview.conflicts?.length ?? 0}</strong><small>现有冲突</small></span><span><strong>{preview.invalidValues.length}</strong><small>无效</small></span></div>{!!preview.conflicts?.length && <div className="access-banner"><span>发现 {preview.conflicts.length} 条规则与现有自定义规则重复或冲突</span><small>继续后将替换对应旧规则；取消可返回修改导入内容</small></div>}<div className="import-preview-list">{preview.rules.map((rule) => { const conflict = preview.conflicts?.find((item) => item.rule.id === rule.id); return <div key={`${rule.type}-${rule.value}`}><span className={`rule-state ${conflict ? '' : 'on'}`}/><span><strong>{rule.value}</strong><small>{rule.type}{conflict ? ` · 将替换 ${conflict.matches.length} 条现有规则` : ''}</small></span></div>; })}{!preview.rules.length && <div className="empty-state compact-empty"><strong>没有可导入的规则</strong><span>请返回修改批量内容</span></div>}</div><div className="bulk-preview-footer"><button onClick={() => setPreview(null)}>取消导入</button><button className="primary-action icon-action" disabled={!preview.rules.length} onClick={confirmImport}><UiIcon name="upload" size={17}/>{preview.conflicts?.length ? `替换并导入 ${preview.rules.length} 条` : `确认导入 ${preview.rules.length} 条`}</button></div></section></div>, document.body)}
    {!!ruleConflicts.length && createPortal(<div className="rules-dialog-backdrop" onMouseDown={() => !replacingRule && setRuleConflicts([])}><section className="rule-delete-dialog rule-conflict-dialog" role="dialog" aria-modal="true" aria-labelledby="rule-conflict-title" onMouseDown={(event) => event.stopPropagation()}><span className="action-dialog-icon orange"><UiIcon name="info" size={24}/></span><div><h2 id="rule-conflict-title">发现重复或冲突规则</h2><p>新规则会与以下现有自定义规则产生重复匹配。替换后，旧规则将被移除并在当前分类保存新规则。</p><div className="import-preview-list">{ruleConflicts.map((conflict) => <div key={conflict.id}><span className={`rule-state ${conflict.kind === 'duplicate' ? 'on' : ''}`}/><span><strong>{conflict.type} · {conflict.value}</strong><small>{conflict.categoryName} · {conflict.reason}</small></span></div>)}</div></div><div className="action-dialog-actions"><button disabled={replacingRule} onClick={() => setRuleConflicts([])}>取消</button><button className="primary-action" disabled={replacingRule} onClick={() => add(true)}>{replacingRule ? '正在替换…' : '替换'}</button></div></section></div>, document.body)}
